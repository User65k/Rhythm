use crate::{
    list::{
        adjust_spacer_h, clear_row, get_list_bottom_spacer, get_list_ref, get_list_top_spacer,
        get_scroll_ref,
    },
    log, show_error_w_val, WebRes,
};
use wasm_bindgen::prelude::*;
use wasm_bindgen::JsCast;
use web_sys::{
    AddEventListenerOptions, Element, Event, HtmlElement, IntersectionObserver,
    IntersectionObserverEntry, IntersectionObserverInit, NodeList,
};
...
fn inv_scroll(e: Element, obs: &IntersectionObserver) -> WebRes {
    ...
    if *top == e {
        ...
        if off > 0 {
            //first is not at the top -> we can load more
...
pub fn setup_inf_scroll(list_root: &Element) -> WebRes {
    ...
    let obs_fn =
        Closure::wrap(Box::new(load_scroll) as Box<dyn Fn(NodeList, IntersectionObserver)>);
