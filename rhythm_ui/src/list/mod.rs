use crate::{ctx::add_ctx_men, get_document_ref, log, WebRes};
use wasm_bindgen::prelude::*;
use wasm_bindgen::JsCast;
use web_sys::{window, Document, Element, HtmlElement, Response, Url};
...
fn get_list_ref() -> &'static HtmlElement {
    unsafe { LIST_ELE.as_ref().expect("No list root") }
}
/// return the element that scrolls the list. (Is a div)
fn get_scroll_ref() -> &'static HtmlElement {
    unsafe { SCROL_ELE.as_ref().expect("No scroll ele") }
}
...
fn get_list_top_spacer() -> &'static HtmlElement {
    unsafe { TOP_SPACER.as_ref().expect("No list root") }
}
fn get_list_bottom_spacer() -> &'static HtmlElement {
    unsafe { BOTTOM_SPACER.as_ref().expect("No list root") }
}
fn adjust_spacer_h(element: &HtmlElement, h: i32) -> WebRes {
    element.style().set_property(
        "height",
        &format!("{}px", (h * get_item_hight()) + element.offset_height()),
    )?;
...
pub fn setup(document: &Document) -> WebRes {
    ...
    unsafe {
        LIST_ELE = document
            .query_selector(".ilist tbody")?
            .and_then(|oe| oe.dyn_into::<HtmlElement>().ok());
        SCROL_ELE = LIST_ELE
            .as_ref()
